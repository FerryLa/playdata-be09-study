...
let numbers = [];
for (let i = 0; i < 10; i++) {
  numbers[i] = Math.floor(Math.random() * 100) + 1; // 0부터 99까지, + 1을 하면 0부터 100까지
}

console.log(`(문자)정렬 전 numbers : ${numbers}`);
numbers.sort(); // 정렬(오름차순)
console.log(`(문자)정렬 후 numbers : ${numbers}`);
// 문자정렬이기 때문에 숫자 100이 앞으로 맨 앞에 온다.
// ex) 100 -> '1', 70 -> '7'

// sort의 기본 정렬은 문자 정렬이므로 필요하다면 정렬기준 콜백 함수를 전달해야 한다.
...
console.log(numbers);
const odds = numbers.filter((item) => item % 2); // 나머지 1 >>>>> true
...
// Array.prototype.find: 배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백 함수를 실행하여 그 결과가 참인 첫 번째 요소를 반환.
// 참인 요소가 존재하지 않는다면 undefined를 반환.
// Array.prototype.findIndex: 배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백 함수를 실행하여 그 결과가 참인 첫 번째 요소의 인덱스를 반환.
