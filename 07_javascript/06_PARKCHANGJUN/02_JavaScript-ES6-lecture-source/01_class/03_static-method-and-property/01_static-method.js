...
class Student {
  ...
  // 클래스 선언브 안에 위치하고 static키워드를 붙이면 정적메소드가 선언된다.(일반메소드에서 정석메소드로 바뀜)
...
Student.staticMethod = function () {
  console.log(
    `staticMethod는 메서드를 프로퍼티 형대로 직접 할당하는 것과 동일하다.`
  );
};
...
class User {
  ...
  static registsUser(id) {
    return new this(id, new Date());
  }
}

let user01 = User.registsUser("user01");
