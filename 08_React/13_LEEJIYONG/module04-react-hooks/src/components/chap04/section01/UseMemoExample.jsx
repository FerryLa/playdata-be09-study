...
export default function UseMemoExample() {
  ...
  const [nonMemoNumber, setNonNumber] = useState(1);
  ...
  const nonMemoHandler = (e) => {
    setNonNumber(slowFunction(parseInt(e.target.value)));
  };

  return (
    <div>
      <h2>useMemo로 계산 결과 메모리제이션</h2>
      ...
      <hr />
      <input type="text" value={nonMemoNumber} onChange={nonMemoHandler} />
      <p>두배값 : {slowFunction(nonMemoNumber)}</p>
    </div>
  );
}
