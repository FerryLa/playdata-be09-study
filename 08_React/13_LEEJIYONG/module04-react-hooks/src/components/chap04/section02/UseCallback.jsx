...
// React.memo : 함수를 React.memo로 래핑되면 React는 컴포넌트를 랜더링하고 결과를 메모라이징 할 때 사용
const ButtonA = React.memo(({ onClick }) => {
  console.log("props 변경으로 인해 ButtonA렌더링");
  return <button onClick={onClick}>일반 사용</button>;
});
...
// value가 변경되면 buttonA는 렌더링되지만 buttonB는 렌더링되지 않는다.
// 이는 useCallback을 사용하면 함수를 메모리제이션하여 함수를 재사용하기 때문이다.
// (console을 확인해봄)
