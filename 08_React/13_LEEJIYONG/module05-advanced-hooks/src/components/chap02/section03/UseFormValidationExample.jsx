/*
  [useFormValidation]
  - 폼의 입력값의 유효성을 검사하고, 에러메시지를 반환하는 커스텀 hook
...
function useFormValidation(initialValues, validate) {
  const [values, setValuees] = useState(initialValues);
  const [errors, setErrors] = useState([]);

  const handleChange = (e) => {
    const { name, value } = e.target;
    //입력값 업데이트
    setValuees((prev) => ({ ...prev, [name]: value }));
...
function validate(name, value) {
  if (name === "email" && !value.includes("@")) return "이메일형식이 아니빈다.";
